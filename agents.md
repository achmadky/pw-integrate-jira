...
### Step 2: Generate Detailed Manual Test Cases (CSV)
- **Rules & Requirements:**
...
### Step 3: Explore and Verify via Playwright Browser Automation (MCP / Browser)
- Use browser automation / Playwright MCP to actively visit the target URL (`https://bantuan-kita.vercel.app/` etc.).
...
- **Rules:**
  1. Build the Playwright test script (`tests/{issueKey}.spec.ts`) strictly mapped to the detailed steps and empirical findings from the updated CSV test cases.
  ...
  3. Avoid generic placeholders (`example.com`) or empty test implementations; write fully realized, actionable test steps matching the app's real DOM structure.
