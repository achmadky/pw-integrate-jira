...
test.describe('KAN-9: Search Feature End-to-End Tests', () => {
  ...
  test('KAN-TC-20: Search for Grey and verify Grey jacket appears in results', { tag: '@KAN-TC-20' }, async ({ searchPage, catalogPage }, testInfo) => {
    ...
    await test.step('Step 4: Save proof screenshot', async () => {
      const screenshotPath = path.resolve('test-results', 'KAN-TC-20-proof.png');
      await searchPage.page.screenshot({ path: screenshotPath, fullPage: true });
      await testInfo.attach('KAN-TC-20-proof.png', { path: screenshotPath, contentType: 'image/png' });
    });
  });

  test('KAN-TC-21: Search with non-existent query and verify zero results handled gracefully', { tag: '@KAN-TC-21' }, async ({ searchPage, catalogPage }, testInfo) => {
    await test.step('Step 1: Open storefront', async () => {
      await catalogPage.openStorefront(TEST_DATA.storeUrl);
    });

    await test.step('Step 2: Enter non-existent query and submit', async () => {
      await searchPage.performSearch('NonExistentProductXYZ');
      await expect(searchPage.page).toHaveURL(/search\?.*q=NonExistentProductXYZ/i);
    });
    ...
    await test.step('Step 4: Save proof screenshot', async () => {
      const screenshotPath = path.resolve('test-results', 'KAN-TC-21-proof.png');
      await searchPage.page.screenshot({ path: screenshotPath, fullPage: true });
      await testInfo.attach('KAN-TC-21-proof.png', { path: screenshotPath, contentType: 'image/png' });
    });
  });
